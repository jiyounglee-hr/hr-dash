...
def get_sharepoint_data():
    try:
        ...
        st.error(f"SharePoint 데이터를 불러오는 중 오류가 발생했습니다: {str(e)}")
        # 로컬 파일로 폴백
        return pd.read_excel("임직원 기초 데이터.xlsx")
...
# 파일 경로 설정 및 데이터 로드
file_path = "임직원 기초 데이터.xlsx"  # 엑셀 파일은 app.py와 같은 디렉토리에 위치
