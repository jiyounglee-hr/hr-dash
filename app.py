...
# 조회 조건 컬럼 생성
                ...
                with col1:
                    ...
                    if selected_status == '🐯 보고':
                        ...
                        with col3_1:
                            password = st.text_input("비밀번호를 입력하세요", type="password", key="tiger_password")
                            if password:  # 비밀번호가 입력된 경우에만 검증
                                if password != "0328":  # 비밀번호가 틀린 경우
                                    st.error("비밀번호가 올바르지 않습니다.")
                                    st.stop()  # 여기서 실행을 중단
                                else:
                                    st.success("인증되었습니다.")
                            elif password == "":  # 비밀번호가 빈 문자열인 경우 (초기 상태)
                                st.stop()  # 추가 실행을 중단하지만 메시지는 표시하지 않음
